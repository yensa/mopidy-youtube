...
def resolve_url(url, stream=False):
    try:
        video = pafy.new(url)
        if not stream:
            uri = "%s/%s.%s" % (video_uri_prefix, safe_url(video.title), video.videoid)
        else:
...
def search_youtube(q):
    query = {"part": "id", "maxResults": 15, "type": "video", "q": q, "key": yt_key}
    ...
    def search(self, query=None, uris=None, exact=False):
        ...
        if "uri" in query:
            ...
            if "youtube.com" in url.netloc:
                ...
                if "list" in req:
                    return SearchResult(
                        uri=search_uri, tracks=resolve_playlist(req.get("list")[0])
                    )
                else:
            ...
            return SearchResult(uri=search_uri, tracks=search_youtube(search_query))
